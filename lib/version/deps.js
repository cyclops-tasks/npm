...
export async function bumpAndMatchDeps(options) {
  const { behind, dirty } = await status(options)
  const noCommit = behind || dirty

  do {
    await addDeps(options)
    await syncDeps(options)
    ...
    await bumpPublishVersions(options)
    await syncDeps(options)
    await commit(options)
  } while (keepTrying(options))
}

export async function matchDeps(options) {
  const { behind, dirty } = await status(options)
  const noCommit = behind || dirty
...
function keepTrying(options) {
  const { events, props } = options
  const { npmInstall, publish, operations } = events.get(
    props
  )
...
async function addDeps(options) {
  const { events, props } = options
