...
// Helpers
import { resetArgv } from "./version/argv"
...
import { gitBehind, gitDirty } from "./version/git"
...
export default function(options) {
  ...
  events.on({
    "before.cyclops.version-tasks.task": resetArgv,

    "cyclops.version-tasks.task": [
      { component, dryMode, readPackageJson },
      phase(store, "Read package json"),
      {
        if: [
          () => !store.get("argv.cyclops.publish"),
          () => [
            addDependencies,
